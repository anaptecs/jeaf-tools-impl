...
/**
 * This class implements the web tools interface for all full featured java environments such as JSE and JEE.
 * 
...
public class WebToolsImpl implements WebTools {
  ...
  private static final Map<String, String> HTML_ESCAPES;

  static {
    String[][] lEscapesMapping = { { "\"", "&quot;" }, // " - double-quote
      { "&", "&amp;" }, // & - ampersand
      { "<", "&lt;" }, // < - less-than
      { ">", "&gt;" }, // > - greater-than
      ...
      // Mapping to escape ISO-8859-1 characters to their named HTML 4.x equivalents.
      // <!-- Latin Extended-B -->
    ...
    Map<String, String> lEscapes = new HashMap<>();
    for (String[] lNextEscape : lEscapesMapping) {
      // takes the html escaped characters as keys
      lEscapes.put(lNextEscape[1], lNextEscape[0]);
    }
    HTML_ESCAPES = Collections.unmodifiableMap(lEscapes);
  }
  ...
  public WebsiteInfo lookupWebsiteInfo( String pURL, int pTimeout, boolean pFollowRedirects,
    ...
    // Execute HTTP GET request to passed URL to gather requested information.
    Connection lConnection = Jsoup.connect(pURL);
    lConnection.timeout(pTimeout);
    lConnection.followRedirects(pFollowRedirects);

    // Accept any SSL certificate.
    if (pAcceptAllSSLCerts == true) {
      SSLSocketFactory lSSLSocketFactory = Tools.getSSLTools().getAcceptAllCertsSSLSocketFactory();
      lConnection.sslSocketFactory(lSSLSocketFactory);
      XFun.getTrace().warn(
          "Performing lookup for website information with insecure SSL Socket Factory. Please be aware that this might cause serious security issues.");
    }
    try {
      // Try to lookup information about website
      Response lResponse = lConnection.execute();
      Document lWebsite = lResponse.parse();

      // Collect all information and return them.
      return new WebsiteInfoImpl(lWebsite.location(), lWebsite.title(),
          HTTPStatusCode.fromStatusCode(lResponse.statusCode()),
          XFrameOptions.fromHTTPHeader(lResponse.header("X-Frame-Options")), lResponse.headers(), lResponse.cookies());
    }
    catch (IOException e) {
      throw new JEAFSystemException(ToolsMessages.UNABLE_TO_LOOKUP_WEBSITE_INFORMATION, e, pURL);
    }
  }
  ...
  public boolean isValidURL( String pURL ) {
    ...
    // null URLs are treated as invalid.
    else {
      lValidURL = false;
    }
    return lValidURL;
  ...
  public String unescapeHtml( String pInput ) {
    // Check parameters.
    Check.checkInvalidParameterNull(pInput, pInput);

    // Replace all e
    String lUnescapedString = pInput;

    for (Entry<String, String> lNextEntry : HTML_ESCAPES.entrySet()) {
      String lKey = lNextEntry.getKey();
      if (lUnescapedString.contains(lKey)) {
        String lValue = lNextEntry.getValue();
        lUnescapedString = lUnescapedString.replaceAll(lKey, lValue);
      }
    }
    return lUnescapedString;
  }
  ...
  public String createBasicAuthorizationHeader( String pUser, String pPassword ) {
    ...
    if (pUser != null) {
      lBuilder.append(pUser);
    }
    lBuilder.append(':');
    if (pPassword != null) {
      lBuilder.append(pPassword);
    }
    byte[] lBase64Credentials = Base64.getEncoder().encode(lBuilder.toString().getBytes(StandardCharsets.UTF_8));
    return "Basic " + new String(lBase64Credentials);
  }
